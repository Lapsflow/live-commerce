...
export const POST = withRole(['MASTER'], async (req: NextRequest) => {
  try {
    ...
    const { start_date, end_date, sub_domain_seq } = validation.data;

    console.log(`Starting ONEWMS order import: ${start_date} ~ ${end_date}`);
    ...
    return ok({
      message: 'ONEWMS order import completed',
      orders: {
        total: result.total,
