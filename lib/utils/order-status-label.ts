...
export function getOrderStatusLabel(combo: OrderStatusCombo): string {
  ...
  // APPROVED + PAID
  if (shippingStatus === "SHIPPED") return "배송완료";
  if (shippingStatus === "PARTIAL") return "부분출고";
  if (shippingStatus === "PREPARING") return "배송준비중";
...
const statusColorMap: Record<OrderStatusLabelType, string> = {
  "발주요청": "bg-yellow-100 text-yellow-800",
  "입금대기": "bg-blue-100 text-blue-800",
  "입금완료": "bg-green-100 text-green-800",
  "배송준비중": "bg-indigo-100 text-indigo-800",
